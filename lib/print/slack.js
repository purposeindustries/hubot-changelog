...
function zero(options) {
  return {
    ...
    content: {
      title: format(
        'There isnt any releases in <https://github.com/%s/%s|%s/%s>',
        options.user,
        options.repo,
        options.user,
        options.repo),
...
function single(rls, options) {
  return {
    ...
    content: {
      text: issuer(rls.body, options),
      title: format('The latest release is %s', rls.tag_name),
      title_link: rls.url,
      fields: [{
        title: 'Project',
        value: format(
...
function range(releases, options) {
  return {
    ...
    content: releases.map(function(rls) {
      return single(rls, options).content;
    })
  };
}
...
function issuer(body, options) {
  return (body || '').replace(/(#\d+)/gm, function(num) {
    return format(
